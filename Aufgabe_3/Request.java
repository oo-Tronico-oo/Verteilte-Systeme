...
class Request {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String rmiHost = null; // Der RMI Hostname
        ...
        while (true) {
            ...
            if (requestValue[2].equals("Server+beenden")) {
                ...
                pw.print(getContent(name, nummer, rmiHost));
            }
            ...
            if (requestValue[2].equals("Server+beenden")) {
                try{
                TelInterface remoteTelServer = (TelInterface)Naming.lookup("rmi://"+rmiHost+"/TelService");
                remoteTelServer.rmiBeenden();
                }catch(Exception e){
                };
    ...
    private static String getContent(String name, String number, String rmiHost) throws Exception {
        ...
        if (nameCheck && numberCheck) {
            ...
            switch (searchID) {
                case 1:
                    temp = remoteTelServer.oneValueSearch(searchName);
                    System.out.println("RMI hat geantwortet");
                    break;
                case 2:
                    temp = remoteTelServer.oneValueSearch(searchNumber);
                    System.out.println("RMI hat geantwortet");
                    break;
                case 3:
                    temp = remoteTelServer.twoValueSearch(searchName, searchNumber);
                    System.out.println("RMI hat geantwortet");
                    break;
                default:
